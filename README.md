...
| 博主搜索 | 部分可用 | `X.com`、`哔哩哔哩` 支持按关键词搜索对应领域最热博主，并全自动评论其最近内容；`LinkedIn` 当前未接入博主搜索 |
...
- `/search/influencers`：搜索某关键词对应领域最热博主，并自动评论其最近帖子
...
`/search/influencers` 页面支持一键执行“搜索博主 + 批量评论”：

- 输入一个关键词，系统会搜索该关键词对应领域最热的博主
- 默认最多处理 `20` 个博主
- 对每个博主默认抓取其最新 `50` 条帖子
- 系统会按顺序逐条生成评论并自动执行
- 当前这套流程支持 `X.com` 和 `哔哩哔哩`
- `LinkedIn` 当前不支持这条自动化链路
